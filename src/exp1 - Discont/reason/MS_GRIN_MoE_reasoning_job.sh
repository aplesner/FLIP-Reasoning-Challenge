...
#SBATCH --nodelist=tikgpu08 # {{NODE}} # choose specific node
#SBATCH --mem=100G
#SBATCH --nodes=1
#SBATCH --gres=gpu:02
...
cp -r /itet-stor/kturlan/net_scratch/project_storage/FLIP/data /scratch/${USER}/FLIP
...
images_dir="${data_dir}/short_images_of_valid_flips"
# one or multiple model (separated by comma & no space in between) \
# names as defined in the params_file
# MS_GRIN_MoE ~ 80_910 MiB at bfloat16
reasoning_models="MS_GRIN_MoE"
# one or multiple model (separated by comma & no space in between) \
...
caption_file_paths="${data_dir}/full_val_split_images__ViPLlava_7B.json"
caption_file_paths="${caption_file_paths}::${data_dir}/full_val_split_images__ViPLlava_13B.json"
caption_file_paths="${caption_file_paths}::${data_dir}/full_val_split_images__LlavaNeXT_mistral_7B.json"
caption_file_paths="${caption_file_paths}::${data_dir}/full_val_split_images__LlavaNeXT_vicuna_7B.json"
caption_file_paths="${caption_file_paths}::${data_dir}/full_val_split_images__LlavaNeXT_vicuna_13B.json"
caption_file_paths="${caption_file_paths}::${data_dir}/full_val_split_images__BLIP2_2_7B.json"
caption_file_paths="${caption_file_paths}::${data_dir}/full_val_split_images__BLIP2_6_7B_COCO,BLIP2_flan_t5_xxl.json"
caption_file_paths="${caption_file_paths}::${data_dir}/full_val_split_images__BLIP2_6_7B_COCO,BLIP2_flan_t5_xxl.json"
caption_file_paths="${caption_file_paths}::${data_dir}/full_val_split_images__Llama_3_2_11B.json"


params_file_path="${data_dir}/params_reason_0.json"
...
filepath="src/LOCAL_reason.py"

echo "running script: $filepath"

# apptainer exec --nv --bind "/scratch/$USER:/scratch/$USER" \
# --env LC_ALL=C,USER=$USER_NAME,TMPDIR=$TMPDIR,PYTHONUSERBASE=$PYTHONUSERBASE,PYTHONNOUSERSITE=$PYTHONNOUSERSITE,PIP_CACHE_DIR=$PIP_CACHE_DIR,PYTHONPATH=$PYTHONPATH,JUPYTER_DATA_DIR=$JUPYTER_DATA_DIR,HF_HOME=$HF_HOME,TRANSFORMERS_CACHE=$TRANSFORMERS_CACHE,HUGGINGFACE_HUB_CACHE=$HUGGINGFACE_HUB_CACHE,TORCH_HOME=$TORCH_HOME,PYTORCH_NVCC_FLAGS="-Xcompiler -fPIC -D__CUDA_NO_HALF2_OPERATORS__" \
# /scratch/$USER/cuda_sandbox /scratch/$USER/apptainer_env/venv/bin/python -m pip install --upgrade setuptools wheel
