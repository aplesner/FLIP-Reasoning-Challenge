...
def process_flip_batch(flip_data_batch=None, image_captions = None, caption_model=None, images_dir=None, input_format='single_sentence') -> Dict[str, str]:
    ...
    if TEST_MODE:
        print("processing a batch of flips")
    ...
    try:
        for model in ref_storage:
            ...
            if caption_model=="built_in" and not ref_storage[model]["support_image_input"]:
                batch_results[model] = {caption_model: "not supported"}
                continue
            batch_results[model] = {caption_model: {}}
            ...
            for idx, model_resp in enumerate(batched_model_resps):
                guess = format_output(model_resp, batched_model_name[idx])
                acc = guess==flip_data_batch[idx]["agreed_answer"][0]
                batch_results[batched_model_name[idx]][caption_model][flip_data_batch[idx]["name"]] = {
                    "acc":acc,
                    "guess":guess
                }
...
def collate_fn(batch):
    # The batch is already a list of dictionaries (one dictionary per batch item).
    # We return it as-is to keep each dictionary intact.
    return batch
...
def main():
    ...
    if TEST_MODE:
        print("TEST_MODE")
        flip_challenge_files=flip_challenge_files[:2*BATCH_SIZE]

    flip_data_loader = DataLoader(FlipChallengeDataset(flip_challenge_files), batch_size=BATCH_SIZE, shuffle=True, collate_fn=collate_fn)

    for cnt, flip_data_batch in tqdm(enumerate(flip_data_loader), total=len(flip_data_loader), desc=f"over flips"):
        
        if TEST_MODE:
            print(f"flip_data: {flip_data_batch}")    
        ...
        for caption_model in caption_models:   
            ...
            if False :#flip_data["name"] in solutions[reasoning_model][caption_model]["flips_res"]:
                ...
                new_res = process_flip_batch(flip_data_batch=flip_data_batch, image_captions=image_captions, caption_model=caption_model, images_dir=images_dir)
                ...
                for reasoning_model in new_res:
                    ...
                    # solutions[reasoning_model][caption_model]["flips_cnt"] += len(flip_data_batch)
                    for flip in flip_data_batch:
                        solutions[reasoning_model][caption_model]["total_acc"] += new_res[reasoning_model][caption_model][flip["name"]]["acc"]
